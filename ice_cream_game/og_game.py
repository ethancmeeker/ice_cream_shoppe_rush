...
containers = [assets.chocolate_sprinkles_jar, assets.rainbow_sprinkles_jar, assets.confetti_sprinkles_jar, assets.chocolate_syrup_container,
            assets.caramel_syrup_container, assets.strawberry_syrup_container, assets.nut_can, assets.cookie_bit_can, assets.marshmellow_can]
symbols = [assets.chocolate_sprinkles_symbol, assets.rainbow_sprinkles_symbol, assets.confetti_sprinkles_symbol, assets.chocolate_syrup_symbol,
            assets.caramel_syrup_symbol, assets.strawberry_syrup_symbol, assets.nuts_symbol, assets.cookie_bits_symbol, assets.marshmellows_symbol]
...
class Ice_Cream_Game(arcade.Window):
    ...
    def __init__(self):
        ...
        self.ic_layer = 295 # Where the ice cream will go when clicked
    ...
    def on_draw(self):
        ...
        if self.main_game_active == True:
            ...
            arcade.draw_text(score_text, 10, 270, arcade.csscolor.BLACK, 18)  # Draws the score
            score_text = (f"Customers Left: {self.customers_left}")
            arcade.draw_text(score_text, 400, 260, arcade.csscolor.BLACK, 16) # Draws the customers left
    ...
    def on_mouse_press(self, x, y, button, modifiers): # Mouse press 
        if self.main_game_active == False: # main menu
            ...
            def main_game():
                ...
                backdrop = arcade.Sprite(assets.backdrop, 1.5)
                ...
                counter = arcade.Sprite(assets.counter, 1.5)
                counter.position = [SCREEN_WIDTH/2, SCREEN_HEIGHT - 443]
                self.scene.add_sprite_list_before("counter", "ice_cream_bin", False, counter)
                timer = arcade.Sprite(assets.timer, 2)
                timer.position = [SCREEN_WIDTH/6, SCREEN_HEIGHT - 35]
                ic_cone = arcade.Sprite(assets.cone, 3)
                ic_cone.position = [350, 245]
            ...
            if self.how_to_play == False:
                ...
                if SCREEN_WIDTH/2 in range(self.mouse_x - 100, self.mouse_x + 100):
                    if 200 in range(self.mouse_y - 50, self.mouse_y + 50):
                        ...
                        self.main_game_active = True
                        self.satisfied_customer = True
                        self.music = assets.master_music
                        main_game()
            ...
            if self.how_to_play == True:
                if 575 in range(self.mouse_x - 40, self.mouse_x + 40):
                    if 540 in range(self.mouse_y - 40, self.mouse_y + 40):
                ...
                if bin_x_locations[i] in range(self.mouse_x - 35, self.mouse_x + 35):
                    for j in range(len(bin_y_locations)):
                        if bin_y_locations[j] in range(self.mouse_y - 35, self.mouse_y + 35):
                            scooped = (flavors[(j * 4) + i])
                            scoop = arcade.Sprite(
                                scooped, 3
                            )
                            scoop.position = [350, self.ic_layer]
                            self.ic_layer += 17
                            l.append(scoop.position)
                            self.scene.add_sprite("scoops", scoop)
                            self.cone.append(flavors[(j * 4) + i])
                            self.new_scoop = True
                            self.scoop = scooped
            for i in range(len(topping_x_locations)):
    ...
    def on_update(self, delta_time):   
        ...
        if self.main_game_active == True:
            ...
            if self.timer_active == True: # Makes the timer go
                ...
                timer_bar.position = [self.symbol_start, SCREEN_HEIGHT - 45]  
            ...
            if self.in_store == False: # Checks if a customer is in the shoppe or not
                ...
                ic1 = arcade.Sprite(flavors[scoop1], 2.2)
                ic1.position = [SCREEN_WIDTH/2 + 150, SCREEN_HEIGHT/2 + 230]
                self.scene.add_sprite("wanted", ic1)
                ic2 = arcade.Sprite(flavors[scoop2], 2.2)
                ic2.position = [SCREEN_WIDTH/2 + 200, SCREEN_HEIGHT/2 + 230]
                self.scene.add_sprite("wanted", ic2)
                ic3 = arcade.Sprite(flavors[scoop3], 2.2)
                ic3.position = [SCREEN_WIDTH/2 + 250, SCREEN_HEIGHT/2 + 230]
                self.scene.add_sprite("wanted", ic3)
                t = arcade.Sprite(symbols[topping], 3)
                t.position = [SCREEN_WIDTH/2 + 200, SCREEN_HEIGHT/2 + 170]
                self.scene.add_sprite("wanted", t)
                customer = arcade.Sprite(customers[self.customer_choice], 1.5)
                customer.position = [SCREEN_WIDTH/2 + 20, SCREEN_HEIGHT/2 + 25]
                bubble = arcade.Sprite(assets.order_bubble, 1.7)
                bubble.position = [SCREEN_WIDTH/2 + 200, SCREEN_HEIGHT/2 + 200]
            ...
            if self.new_scoop == True: # Will check if you are adding a new scoop/topping to the cone
                ...
                # Either you put the wrong ice cream/topping on the cone or you run out of time
                    ...
                    self.scene.add_sprite_list_after("bubble", "customers", False)
                    self.scene.add_sprite_list_after("wanted", "bubble", False)
                    customer = arcade.Sprite(negative_customers[self.customer_choice], 1.5)
                    customer.position = [SCREEN_WIDTH/2 + 20, SCREEN_HEIGHT/2 + 25]
                    self.scene.add_sprite("customers", customer)             
            ...
            if len(self.cone) >= 4: # This may not look like it but it checks if the cone is right or not
                ...
                self.scene.add_sprite_list_after("bubble", "customers", False)
                self.scene.add_sprite_list_after("wanted", "bubble", False)
                customer = arcade.Sprite(positive_customers[self.customer_choice], 1.5)
                customer.position = [SCREEN_WIDTH/2 + 20, SCREEN_HEIGHT/2 + 25]
                self.scene.add_sprite("customers", customer)
                self.display_emotion += delta_time
                if self.display_emotion >= 2.0:
